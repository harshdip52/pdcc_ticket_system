 ...
 $login_role = $this->session->userdata('login_role');
   ...
   if($login_role==5 || $login_role==9 && $ticket['status']!='Resolved' && $login_user== $ticket['user_id']){ ?>
<!--  -->
    ...
    <form method="post" action="<?=base_url()?>Support/forwardTicket/<?=$ticket['ticket_id'] ?>" id="forwardTicket">
      ...
      <form method="post" action="<?=base_url()?>Support/ticketReplyNew/<?=$ticket['ticket_id'] ?>" id="ticketReply">
        <input type="hidden" name="ticket_id" value="<?=$ticket['ticket_id'] ?>">
        <div class="row">
          <div class="col-md-12">
            <div class="row">
