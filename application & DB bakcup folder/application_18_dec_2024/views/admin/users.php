    ...
    <form method="post" action="<?= base_url() ?>admin/users" id="add_users">
      <div class="panel-body">
        <div class="row">
          ...
          <div class="col-md-4 input-padd">
            ...
            <input class="form-control" type="text" name="emp_id" id="emp_id" autocomplete="off">
            <span class="text-danger" id="emp_id_msg"></span>
          </div>
      ...
      <div class="panel-heading" style="background-color: #0e6d8c;">
        <div class="panel-heading-btn" style="margin-top: -4px;">
          <button class="btn btn-success btn-sm text-white btnsubmitdata" data-toggle="modal" data-target="#" id="btnsubmitdata">Submit</button>
...
<script>
  ...
  $('#btnsubmitdata').click(function(e) {
    ...
    //alert("error checking please wait.....");
    //return;
  ...
  $("#emp_id").on('change paste', function() {
  // $("#emp_id").on('input propertychange', function() {
    var emp_id = $("#emp_id").val();
    // $("#emp_id").html("");
    if(emp_id != '' || emp_id != null){
        $.ajax({
          url: "<?= base_url() ?>Admin/chechDuplicateEmpId",
          type: "post",
          dataType: 'json',
          data: {
            emp_id: btoa(emp_id)
          ...
          success: function(result) {
            if (Object.entries(result).length > 0) {              
              $("#emp_id_msg").html("Duplicate Emp Id  Found In System");
              // swal("Error", "Duplicate Emp Id  Found In System", "error");
              $("#btnsubmitdata").prop('disabled', true);
              // $(".btnsubmitdata").addClass('disabled');
              return;
            } else {
              $("#btnsubmitdata").prop('disabled', false);
              // $(".btnsubmitdata").removeClass('disabled');
              return;
            }
          }
        });        
