...
class BranchManagerController extends CI_Controller
{
    public function __construct()
    {
        ...
        $this->load->model("BranchManagerModel");
    ...
    {
        $manager_assign_branch_ids = $this->session->userdata('login_branch_id');
        
        $data['AllTaluka'] = $this->InventoryModel->getAllTalukaList();
        $allusersList = $this->BranchManagerModel->allusers($manager_assign_branch_ids);
        $login_user_branch_id = $this->session->userdata('login_branch_id');
        $newUSersList = [];
        foreach($allusersList as $userkey =>$user){
            if(in_array($login_user_branch_id,explode(",",$user['branch_id']))){                
                $newUSersList[$user['user_id']] = $user['name'];
            }
        }        
        $data['allusers'] = $newUSersList;
        ...
        $this->load->view('manager/managertTicketView', $data);
        $this->load->view('footer/footer');
    }

    function getDetails(){
        $result = $this->BranchManagerModel->getDetails($this->input->post());
    ...
    { 
        ...
        if(count($user_assign_branch_name) > 1){
            $getBranch=$this->BranchManagerModel->getBranch_new($zone_id,$user_assign_branch); 
        }else{
            $getBranch=$this->BranchManagerModel->getBranch($zone_id); 
        }
        echo json_encode($getBranch);
    }
    public function getBranchAjaxByUser()
    { 
        $getBranch=$this->BranchManagerModel->getBranchAjaxByUser(); 
        echo json_encode($getBranch);
    }
    ...
    function getCategoryWiseTitle(){
        ...
        $categoryWiseTitle=$this->BranchManagerModel->getCategoryWiseTitle($catId,$subCatId); 
    ...
    public function getInventoryStock()
    {
        ...
        $AllInventory=$this->BranchManagerModel->getInventoryBySearch($taluka_id, $zone_id,$branch_id); 
    ...
    function addnewTicketAjax()
    {
        ...
        $module_id = 1;
        $CounterData = $this->BranchManagerModel->getCounter($module_id);
        ...
        //     $upload_data = $this->upload->data();
            ...
            $id = $this->BranchManagerModel->addnewTicketAjax($formdata);

            $counter = $CounterData['counter'] + 1;
            $Counterformdata['counter'] = $counter;
            $this->BranchManagerModel->updateCounter($Counterformdata, $module_id);
            if(!empty($id)) {
    ...
    function getTicketTitle(){
        $result = $this->BranchManagerModel->getTicketTitle($this->input->post());
        echo json_encode($result);
        exit(1);
    }
    ...
    function getTalukaByBranch(){
        ...
        $result = $this->BranchManagerModel->getTalukaByBranch($talukaId,$zoneId);
        echo json_encode($result);
    }
    ...
    { 
        $getBranch=$this->BranchManagerModel->getSubCategory($cat_id); 
        echo json_encode($getBranch);
    }
    ...
    public function getInventoryDeadStock()
    {
        ...
        $branch_id=$this->input->post('branch_id');
        $zone_id=$this->input->post('zone_id'); 
        $AllInventory=$this->BranchManagerModel->getInventoryDeadBySearch($taluka_id, $zone_id, $branch_id); 
        echo json_encode($AllInventory);
    ...
    {   
        $id = $this->InventorBranchManagerModelyModel->deleteInventory($id);
        if ($id) 
        {
    ...
    { 
        ...
        $getticket=$this->BranchManagerModel->getTodaysTicket($user_id); 
