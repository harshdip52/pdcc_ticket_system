...
class Admin extends  CI_Controller
{
    ...
    public function add_sub_category()
    {
        if ($this->form_validation->run('add_sub_category') == FALSE) {
            ...
            $formdata['use_for_inventory']   = "Yes";
            $formdata['use_for_ticket']      = "Yes";
            $formdata['status']              = "Active";
    ...
    public function chechDuplicateEmpId()
    {
        $emp_id = base64_decode(($this->input->post('emp_id')));

        $response = $this->AdminModel->chechDuplicateEmpId($emp_id);
        if (!empty($response)) {
    ...
    function changePassword()
    {
        ...
        if ($newPassword == $confirm_newPassword) {
            ...
            $changedPAssword = $this->AdminModel->changePassword($user_id, $newPassword, $confirm_newPassword);

            if ($changedPAssword) {
                return array('status' => true, 'message' => "password updated successfully", 'code' => 200);
            } else {
                return array('status' => false, 'message' => "error while updating password", 'code' => 500);
            }
        } else {
            return array('status' => false, 'message' => "new password and confirm password do not match", 'code' => 500);
        }
    }
    ...
    public function edit_users($user_id = '')
    {
        ...
        if ($this->form_validation->run('users') == FALSE) {
            ...
            $formdata['username']   = $this->input->post('emp_id');
            $formdata['password']   = $this->input->post('mobile');
            $formdata['status']     = $this->input->post('user_status');
    ...
    {
        $ticket_id = $this->input->post('foreclosureTicket_ticket_id');
        $ticket_no = $this->input->post('foreclosureTicket_ticket_no');
        $ticket_remark = $this->input->post('foreclosureTicket_remark');
        $ticket_status = $this->input->post('foreclosureTicket_status');

        $where = array('ticket_id' => $ticket_id, 'ticket_no' => $ticket_no);
        $update = array('status' => $ticket_status, 'remark' => $ticket_remark,'updated_on'=> date('Y-m-d'));

        $result = $this->AdminModel->foreclosureTicketByAadmin($where, $update);
        if ($result) {
            $response = array('status' => true, 'message' => "Ticket No - ".$ticket_no." Is Foreclosure By Admin ", 'code' => 200);
            header('Content-Type: application/json');
            echo json_encode($response);
            exit(1);
        } else {
            $response = array('status' => false, 'message' => "Error While Foreclosure Closing ticket", 'code' => 302);
            header('Content-Type: application/json');
            echo json_encode($response);
            exit(1);
        }
