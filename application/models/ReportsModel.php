...
class ReportsModel extends CI_Model
{
    ...
    public function getInventoryReports($from_date, $to_date, $cat_id, $sub_cat_id, $taluka_id, $zone_id, $branch_id, $status)
    {
        ...
        if ($cat_id) {
            $condition .= ' and  tbl_mst_inventory.cat_id=' . $cat_id;
        }
        if ($sub_cat_id) {
            $condition .= ' and tbl_mst_inventory.sub_cat_id=' . $sub_cat_id;
        }
        if ($taluka_id) {
            $condition .= ' and  tbl_mst_inventory.taluka_id=' . $taluka_id;
        }
        if ($zone_id) {
            $condition .= ' and tbl_mst_inventory.zone_id=' . $zone_id;
        }
        if ($branch_id) {
            $condition .= ' and tbl_mst_inventory.branch_id=' . $branch_id;
        }
        if ($status) {
            $condition .= ' and tbl_mst_inventory.status=' . $status;
        }

        if ($from_date && empty($to_date)) {
            $condition .= ' and tbl_mst_inventory.inserted_on >' . "'" . $from_date . "'";
        }
        if (empty($from_date) && $to_date) {
            $condition .= ' and tbl_mst_inventory.inserted_on < ' . "'" . $to_date . "'";
        }
        if (!empty($from_date) && !empty($to_date) && $from_date == $to_date) {
            $condition .= ' and cast(tbl_mst_inventory.inserted_on as date) =  ' . "'" . $from_date . "'";
        }
        if (!empty($from_date) && !empty($to_date) && $from_date != $to_date) {
            $condition .= ' and tbl_mst_inventory.inserted_on BETWEEN ' . "'" . $from_date . "'" . 'and' . "'" . $to_date . "'";
        }
    ...
    public function getTicketReports($from_date, $to_date, $cat_id, $sub_cat_id, $taluka_id, $zone_id, $branch_id, $user_id, $status)
    {
        ...
        // $condition='id IS NOT null';
        if ($cat_id) {
            $condition .= ' and  tbl_mst_ticket.cat_id=' . $cat_id;
        }
        if ($sub_cat_id) {
            $condition .= ' and tbl_mst_ticket.sub_cat_id=' . $sub_cat_id;
        }
        if ($taluka_id) {
            $condition .= ' and  tbl_mst_ticket.taluka_id=' . $taluka_id;
        }
        if ($zone_id) {
            $condition .= ' and tbl_mst_ticket.zone_id=' . $zone_id;
        }
        if ($branch_id) {
            $condition .= ' and tbl_mst_ticket.branch_id=' . $branch_id;
        }
        if ($user_id) {
            $condition .= ' and tbl_mst_ticket.user_id=' . $user_id;
        }
        if ($status) {
            $condition .= ' and tbl_mst_ticket.status=' . "'" . $status . "'";
        }

        if ($from_date && empty($to_date)) {
            $condition .= ' and tbl_mst_ticket.created_on >' . "'" . $from_date . "'";
        }
        if (empty($from_date) && $to_date) {
            $condition .= ' and tbl_mst_ticket.created_on < ' . "'" . $to_date . "'";
        }
        if (!empty($from_date) && !empty($to_date) && $from_date == $to_date) {
            $condition .= ' and  cast(tbl_mst_ticket.created_on as date) = ' . "'" . $from_date . "'";
        }
        if (!empty($from_date) && !empty($to_date) && $from_date != $to_date) {
            $condition .= ' and tbl_mst_ticket.created_on BETWEEN ' . "'" . $from_date . "'" . 'and' . "'" . $to_date . "'";
        }
    ...
    public function getcallLogReports($from_date, $to_date, $user_id)
    {
        $condition = '';
        if ($user_id) {
            $condition .= ' and  tbl_mst_call_logs.created_by=' . $user_id;
        }
        if ($from_date && empty($to_date)) {
            $condition .= ' and tbl_mst_call_logs.created_at >' . "'" . $from_date . "'";
        }
        if (empty($from_date) && $to_date) {
            $condition .= ' and tbl_mst_call_logs.created_at < ' . "'" . $to_date . "'";
        }


        if (!empty($from_date) && !empty($to_date) && $from_date == $to_date) {
            $condition .= ' and  cast(tbl_mst_call_logs.created_at as date) = ' . "'" . $from_date . "'";
        }
        if (!empty($from_date) && !empty($to_date) && $from_date != $to_date) {
            $condition .= ' and tbl_mst_call_logs.created_at BETWEEN ' . "'" . $from_date . "'" . 'and' . "'" . $to_date . "'";
        }
    ...
    public function getinventoryMovementReports($from_date, $to_date, $cat_id, $sub_cat_id, $taluka_id, $zone_id, $branch_id, $status)
    {
        ...
        // $condition='id IS NOT null';
        if ($cat_id) {
            $condition .= ' and  tbl_inventory_movement_log.cat_id=' . $cat_id;
        }
        if ($sub_cat_id) {
            $condition .= ' and tbl_inventory_movement_log.sub_cat_id=' . $sub_cat_id;
        }
        if ($taluka_id) {
            $condition .= ' and  tbl_inventory_movement_log.taluka_id=' . $taluka_id;
        }
        if ($zone_id) {
            $condition .= ' and tbl_inventory_movement_log.zone_id=' . $zone_id;
        }
        if ($branch_id) {
            $condition .= ' and tbl_inventory_movement_log.branch_id=' . $branch_id;
        }
        // if($status) { $condition.=' and tbl_inventory_movement_log.status='.$status;}

        if ($from_date && empty($to_date)) {
            $condition .= ' and tbl_inventory_movement_log.inserted_on >' . "'" . $from_date . "'";
        }
        if (empty($from_date) && $to_date) {
            $condition .= ' and tbl_inventory_movement_log.inserted_on < ' . "'" . $to_date . "'";
        }


        if (!empty($from_date) && !empty($to_date) && $from_date == $to_date) {
            $condition .= ' and cast(tbl_inventory_movement_log.inserted_on as date) =  ' . "'" . $from_date . "'";
        }
        if (!empty($from_date) && !empty($to_date) && $from_date != $to_date) {
            $condition .= ' and tbl_inventory_movement_log.inserted_on BETWEEN ' . "'" . $from_date . "'" . 'and' . "'" . $to_date . "'";
        }
    ...
    public function getTicketBystatus2($taluka_id, $status, $from_date, $to_date)
    {
        ...
        if ($from_date && empty($to_date)) {
            $condition .= ' tbl_mst_ticket.created_on >' . "'" . $from_date . "'";
        }
        if (empty($from_date) && $to_date) {
            $condition .= ' tbl_mst_ticket.created_on < ' . "'" . $to_date . "'";
        }
        if (!empty($from_date) && !empty($to_date) && $from_date == $to_date) {
            $condition .= ' cast(tbl_mst_ticket.created_on as date) = ' . "'" . $from_date . "'";
        }
        if (!empty($from_date) && !empty($to_date) && $from_date != $to_date) {
            $condition .= ' tbl_mst_ticket.created_on BETWEEN ' . "'" . $from_date . "'" . 'and' . "'" . $to_date . "'";
        }
    ...
    public function getassignInventoryReports($from_date, $to_date, $cat_id, $sub_cat_id, $taluka_id, $zone_id, $branch_id, $status)
    {
        ...
        if ($from_date && empty($to_date)) {
            $condition .= ' and tbl_mst_inventory.assign_on >' . "'" . $from_date . "'";
        }
        if (empty($from_date) && $to_date) {
            $condition .= ' and tbl_mst_inventory.assign_on < ' . "'" . $to_date . "'";
        }
        if (!empty($from_date) && !empty($to_date) && $from_date == $to_date) {
            $condition .= ' and cast(tbl_mst_inventory.assign_on as date) =  ' . "'" . $from_date . "'";
        }
        if (!empty($from_date) && !empty($to_date) && $from_date != $to_date) {
            $condition .= ' and tbl_mst_inventory.assign_on BETWEEN ' . "'" . $from_date . "'" . 'and' . "'" . $to_date . "'";
        }
