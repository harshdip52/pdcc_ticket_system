  ...
  <div class="panel panel-inverse">
    <div class="panel-heading">
      <div class="panel-heading-btn">
        <button class="pull-left btn btn-primary btn-sm " id="" onclick="callDatatable()"><i class="fa">&#xf021;</i></button>
    ...
    <div class="panel-body">
      <div class="row">
        <div class="col-sm-2" id="user_id_div">
          <label for="inputEmail3" class="col-sm-1 col-form-label">Category <sup class="text-danger">*</sup></label>
          <select class="form-select from-font" id="selected_category_id" name="selected_category_id" required="">
            <option value="">Select Category</option>
            <?php foreach ($getAllCategory as $CatKey => $category) { ?>
        ...
        <div class="col-sm-2" id="user_id_div">
          <label for="inputEmail3" class="col-sm-1 col-form-label">Sub&nbsp;Category<br><sup class="text-danger">*</sup></label>
          <select class="form-select from-font" id="selected_sub_category_id" name="selected_sub_category_id" required="">
          </select>
        </div>

        <div class="col-sm-2" id="user_id_div">
          <label style="visibility:hidden;" for="inputEmail3" class="col-sm-1 col-form-label">Sub&nbsp;Category<br><sup class="text-danger">*</sup></label>
          <br>
          <button class="btn btn-primary btn-sm searchIventoyData" id="searchIventoyData" onclick="getInventoryByCategorySubCat()">Search</button>
        </div>
      </div>
    </div>
    ...
    <form method="post" action="<?= base_url() ?>Inventory/assign_inventory" id="assign_inventory">
      <div class="panel-body">
        <div class="row mb-3">
          ...
          <!-- <div class="col-sm-2" id="user_id_div">
            <label for="inputEmail3" class="col-sm-1 col-form-label">Category <sup class="text-danger">*</sup></label>
            <select class="form-select from-font" id="category_id" name="category_id" required="">
              <option value="">Select Category</option>
              <?php foreach ($getAllCategory as $CatKey => $category) { ?>
                <option value="<?php echo $category['cat_id']; ?>"><?php echo $category['cat_name']; ?></option>
              <?php } ?>
          ...
          <div class="col-sm-2" id="user_id_div">
            <label for="inputEmail3" class="col-sm-1 col-form-label">Sub&nbsp;Category<br><sup class="text-danger">*</sup></label>
            <select class="form-select from-font" id="sub_category_id" name="sub_category_id" required="">
            </select>
          </div> -->
            ...
            <table id="eventsTable"
              ...
              <thead>
                <tr>
                  ...
                  <th class="text-nowrap">Receive / Make Date</th>
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    callDatatable(selected_category_id, selected_sub_category_id);
    // callDatatable();
  });

  function getInventoryByCategorySubCat() {
    var selected_category_id = $("#selected_category_id").val();
    var selected_sub_category_id = $("#selected_sub_category_id").val();
    callDatatable(selected_category_id, selected_sub_category_id);
  }
  ...
  $("#selected_category_id").change(function() {
    $("#selected_sub_category_id").html("<option>Select Sub-Category</option>");
    var cat_id = $("#selected_category_id").val()
    $.ajax({
      url: "<?php echo base_url(); ?>inventory/getSubCategory/",
      type: "POST",
      data: {
        cat_id: cat_id
      },
      success: function(response, textStatus, jqXHR) {
        ...
        $.each(JSON.parse(response), function(key, val) {
          html += "<option value='" + val.sub_cat_id + "'>" + val.sub_cat_name + "</option>";
        });
        $("#selected_sub_category_id").html(html);
      },
  ...
  function callDatatable(selected_category_id, selected_sub_category_id) {
    loader();
