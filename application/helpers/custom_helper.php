...
if (!function_exists('userStatus')) {
    ...
    {
        $userStatus =  array('1' => 'Active', '2' => 'Inactive', '3' => 'Blocked');

        return isset($userStatus[$key]) ? $userStatus[$key] : $userStatus;
    }
}
if (!function_exists('getcurrentTimeStamPIST')) {
    function getcurrentTimeStamPIST()
    {
        date_default_timezone_set('Asia/Kolkata');
        $currentDateTime = date('Y-m-d H:i:s');

        $currentTimestamp =  array('date' => date("Y-m-s"), 'time' => date("H:i:s"), 'timestamp' => $currentDateTime);
