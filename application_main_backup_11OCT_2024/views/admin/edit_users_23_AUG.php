...
<script>
  function getBranch(zone_id) {
    ...
    // alert("from branch list "+taluka_id);
    // return;
    $.ajax({
    ...
    /*    $.getJSON("<?php echo base_url(); ?>Ajax/getBranchAjaxEditUser/" + zone_id, function(data) {
          var stringToAppend = "<option disabled value=''>-- Select Branch --</option> ";

          $.each(data, function(key, value) {
            if ($.inArray(value.branch_name.trim(), assign_branch_name_obj) != -1) {
              console.log("Found " + value.branch_name + "\n");
            }
            if ($.inArray(value.branch_name, assign_branch_name_obj) != -1) {
              stringToAppend += "<option selected value='" + value.branch_id + "'>" + value.branch_name + "</option>";
            } else {
              stringToAppend += "<option value='" + value.branch_id + "'>" + value.branch_name + "</option>";
            }
