...
class AdminModel extends CI_Model
{
    ...
    function getAllBranchListTalikaWise($postdata){
    ...
    public function getCallLogs($from_date,$to_date)
    {
        
        $condition = '';       

        if($from_date && empty($to_date)){ $condition.=' WHERE created_at  >'."'".$from_date."'";} 
        if(empty($from_date) && $to_date){ $condition.=' WHERE created_at < '."'".$to_date."'";}
        if(!empty($from_date) && !empty($to_date) && $from_date==$to_date){ $condition.=' WHERE cast(created_at as date) =  '."'".$from_date."'";} 
        if(!empty($from_date) && !empty($to_date) && $from_date!=$to_date){ $condition.=' WHERE created_at BETWEEN '."'".$from_date."'".'and'."'".$to_date."'";} 
        $myQuery = "SELECT * from ticket_support.tbl_mst_call_logs $condition";
       // echo $myQuery;die;
