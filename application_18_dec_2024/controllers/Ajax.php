...
class Ajax extends  CI_Controller
{
    ...
    function moveToHo(){
        ...
        if($inventory_id !='' || $inventory_id !=null){
            ...
            $update = array('branch_id' => $branch_id,'taluka_id' =>'','zone_id'=>'');

            $updateWhere = array('id' => $inventory_id);
            $updateBranch = $this->AjaxModel->moveToHo($update,$updateWhere);
            echo json_encode(array("status"=>false,"message" => "Record With Us!","code"=>500));
        }else{
            echo json_encode(array("status"=>false,"message" => "Record Not With Us!","code"=>500));
        }
    }
    ...
    {   
        ...
        if($status== 3 || $status== 4 || $status== 7)
        {
            ...
            $insertdata['inserted_by']=$this->session->userdata('login_user_id'); 
            $insertdata['inserted_on']=date('Y-m-d H:i:s');
            $insertdata['action_status']=$status;
            $id = $this->InventoryModel->inventory_movement_log($insertdata);
        }
    ...
    { 
        ...
        $AllInventory=$this->AjaxModel->getInventoryForAssign($cat_id,$sub_cat_id); 
        echo json_encode($AllInventory);
