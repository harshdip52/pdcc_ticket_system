  ...
  <div id="content" class="content">
   <ol class="breadcrumb pull-right">
    ...
    <div class="panel panel-inverse">
      ...
      <form method="post" action="<?=base_url()?>admin/edit_branch/<?= $branchdata['branch_id']?>" id="edit_branch">
            <input class="form-control" type="hidden" name="branch_id" id="branch_id" value="<?=$branchdata['branch_id']?>">
          ...
          <div class="col-md-6 input-padd">
            ...
            <select class="form-select from-font" onchange="getZone(this.value)" name="taluka_id" id="taluka_id">
              <?php foreach ($taluka as $key => $talukas) {?>
                ...
                <?php if ($branchdata['taluka_id'] == $talukas['taluka_id'])  echo 'selected = "selected"'; ?>><?=$talukas['taluka_name']?></option> 
              <?php }?>
            </select>
          </div>
          <div class="col-md-6 input-padd">
            ...
            <select class="form-select from-font" name="zone_id" id="zone_id">
              <?php foreach ($zoneList as $key => $zones) {?>

                <option value="<?=$zones['zone_id']?>"
                <?php if ($branchdata['zone_id'] == $zones['zone_id'])  echo 'selected = "selected"'; ?>><?=$zones['zone_name']?></option> 
              <?php }?>
           ...
           <div class="col-md-6 input-padd">
            ...
            <input class="form-control" type="text" name="branch_code" id="branch_code" value="<?=$branchdata['branch_code']?>">
          </div>
          <div class="col-md-6 input-padd">
            <label>Branch Name<sup class="text-danger">*</sup></label>
            <input class="form-control" type="text" name="branch_name" id="branch_name" value="<?=$branchdata['branch_name']?>">
          </div>
        </div> 
      </div>
