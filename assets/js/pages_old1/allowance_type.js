...
$(document).ready(function () {
    ...
    $('#master_form').on('click', '#DoSubmit', function (e) {        
        ...
        var allowance = $("#allowance").val();//select   
        allowance = allowance.trim();
        if (allowance == "") {
            if ($("#allowance").next(".validation").length == 0) // only add if not added
            {
                $("#allowance").after("<div class='validation' style='color:red;margin-bottom:15px;'>Please enter allowance.</div>");    
            }
            allFields = false;
            if (!focusSet) {
                $("#allowance").focus();
            ...
            $("#allowance").next(".validation").remove(); // remove it
        }    
        ///////////////////////////////////////////////
        if (allFields == true) {
    ...
    $('#allRecords').on('click', '#deleteRecord', function (e) {
        ...
        var c = confirm("Are you sure to delete this allowance ?");
        if (c == true) {
            $.post("deleteAllowanceType/", {
                id: id,
            }, function (res) {
                if (res.status == 0) {
                    ...
                    $('.successbox').show().text("Allowance  deleted successfully.");
