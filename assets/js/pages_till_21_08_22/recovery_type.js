...
$(document).ready(function () {
    ...
    $('#master_form').on('click', '#DoSubmit', function (e) {        
        ...
        var recovery = $("#recovery").val();//select   
        recovery = recovery.trim();
        if (recovery == "") {
            if ($("#recovery").next(".validation").length == 0) // only add if not added
            {
                $("#recovery").after("<div class='validation' style='color:red;margin-bottom:15px;'>Please enter recovery.</div>");    
            }
            allFields = false;
            if (!focusSet) {
                $("#recovery").focus();
            ...
            $("#recovery").next(".validation").remove(); // remove it
        }    
        ///////////////////////////////////////////////
        if (allFields == true) {
    ...
    $('#allRecords').on('click', '#deleteRecord', function (e) {
        ...
        var c = confirm("Are you sure to delete this recovery ?");
        if (c == true) {
            $.post("deleteRecoveryType/", {
                id: id,
            }, function (res) {
                if (res.status == 0) {
                    ...
                    $('.successbox').show().text("Recovery  deleted successfully.");
