...
$(document).ready(function () {
    ...
    $("input[type='text'],input[type='email'],#dob").on("keyup", function() {
        //alert("dob");
        if ($(this).val() != "") {
    ...
    $('#user_form').on('click', '#DoSubmit', function (e) {
        ...
        if (!dob.length) {
            if ($("#dob").next(".validation").length == 0) // only add if not added
            {
                $("#dob").after("<div class='validation' style='color:red;margin-bottom:15px;'>Please enter dob.</div>");
            ...
            $("#dob").next(".validation").remove(); // remove it
        }
