...
$(document).ready(function () {
    ...
    $('#master_form').on('click', '#DoSubmit', function (e) {
        ...
        var cause = $("#cause").val();//select   
        cause = cause.trim();
        if (cause == "") {
            if ($("#cause").next(".validation").length == 0) // only add if not added
            {
                $("#cause").after("<div class='validation' style='color:red;margin-bottom:15px;'>Please enter cause.</div>");    
            }
            allFields = false;
            if (!focusSet) {
                $("#cause").focus();
            ...
            $("#cause").next(".validation").remove(); // remove it
        }    
        ///////////////////////////////////////////////
        if (allFields == true) {
            var url = $('#master_form').attr('action');
            //var url ="<?php echo base_url() ?>pensioncell/addNewcause?>";
    ...
    $('#allRecords').on('click', '#deleteRecord', function (e) {
        ...
        var c = confirm("Are you sure to delete this cause ?");
        if (c == true) {
            $.post("deleteCause/", {
                id: id,
            }, function (res) {
                if (res.status == 0) {
                    ...
                    $('.successbox').show().text("Cause  deleted successfully.");
