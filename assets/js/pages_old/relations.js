...
$(document).ready(function () {
    ...
    $('#master_form').on('click', '#DoSubmit', function (e) {
        ...
        var relation = $("#relation").val();//select   
        relation = relation.trim();
        
        if (relation == "") {
            if ($("#relation").next(".validation").length == 0) // only add if not added
            {
                $("#relation").after("<div class='validation' style='color:red;margin-bottom:15px;'>Please enter relation.</div>");    
            }
            allFields = false;
            if (!focusSet) {
                $("#relation").focus();
            ...
            $("#relation").next(".validation").remove(); // remove it
        }    
        ///////////////////////////////////////////////
        if (allFields == true) {
            var url = $('#master_form').attr('action');
            //var url ="<?php echo base_url() ?>pensioncell/addNewrelation?>";
    ...
    $('#allRelations').on('click', '#deleteRecord', function (e) {
        e.preventDefault();
        const $root = $(this);
        var id = $root.data('id');
        
        var c = confirm("Are you sure to delete this relation ?");
        if (c == true) {
            $.post("deleteRelation/", {
                id: id,
            }, function (res) {
                if (res.status == 0) {
                    ...
                    $('.successbox').show().text("relation  deleted successfully.");
