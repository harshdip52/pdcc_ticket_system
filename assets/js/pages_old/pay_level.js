...
$(document).ready(function () {
    ...
    $('#master_form').on('click', '#DoSubmit', function (e) {
        ...
        var paylevel = $("#paylevel").val();//select   
        paylevel = paylevel.trim();
        
        if (paylevel == "") {
            if ($("#paylevel").next(".validation").length == 0) // only add if not added
            {
                $("#paylevel").after("<div class='validation' style='color:red;margin-bottom:15px;'>Please enter pay level.</div>");    
            }
            allFields = false;
            if (!focusSet) {
                $("#paylevel").focus();
            ...
            $("#paylevel").next(".validation").remove(); // remove it
        }    
        ///////////////////////////////////////////////
        if (allFields == true) {
            var url = $('#master_form').attr('action');
            //var url ="<?php echo base_url() ?>pensioncell/addNewpaylevel?>";
    ...
    $('#allPayLevel').on('click', '#deleteRecord', function (e) {
        e.preventDefault();
        const $root = $(this);
        var id = $root.data('id');
        
        var c = confirm("Are you sure to delete this pay level ?");
        if (c == true) {
            $.post("deletePayLevel/", {
                id: id,
            }, function (res) {
                if (res.status == 0) {
                    ...
                    $('.successbox').show().text("pay level  deleted successfully.");
