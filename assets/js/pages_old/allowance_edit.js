...
$(document).ready(function () {
    ...
    $('#edit_form  .errorbox').css('display', 'none');
    $('#edit_form .successbox').css('display', 'none');

    $("#edit_form input[type='text'], textarea,select").on("keyup", function () {
        if ($(this).val() != "" && $("textarea").val() != "" && $("select").val() != 0 && $("#starting_month").val() != "") {
            $("#DoSubmit").removeAttr("disabled");
        } else {
            $("#DoSubmit").attr("disabled", "disabled");
        }
    });
    //     $('#edit_form').on('change', '#starting_month', function (e) {
    //         //alert("dob");
    //        if ($(this).val() != "" ) {
    //            $("#DoSubmit").removeAttr("disabled");
    //        } else {
    ...
    $('#edit_form').on('click', '#DoSubmit', function (e) {
        e.preventDefault();
        $(this).val('Please wait ...')
            .attr('disabled', 'disabled');
        var focusSet = false;
        var allFields = true;
        var pensioner_type = $('input[name=pensioners_type]:checked', '#edit_form').val();
    ...
    var add_amount_by = $('input[type=radio][name=add_amount_by]:checked', '#edit_form').val();
