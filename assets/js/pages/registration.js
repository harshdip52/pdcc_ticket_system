...
$(document).ready(function(){
	...
	$("#u_email").on('change',function(){
		...
		$.get("Registration/checkUniqueEmail", {
			email: email
		},
		function(resp) {
			...
			if(res.status == 0){
				console.log(res);
				if ($("#u_email").next(".validation").length == 0) // only add if not added
				{
				$("#u_email").after("<div class='validation' style='color:red;font-weight:bold;margin-bottom:15px;'>This email already exist.Please enter unique email</div>");				
				}
				$('#email_verify').val(0);				
			}else{
				$("#u_email").next(".validation").remove(); // remove it	
				$('#email_verify').val(1);		
