...
//check size of doc and type of image 
    $('#life_certificate_form #document').on('change', function () {
        ...
        if (this.files[0].size > 2048000) {
            ...
            {
                $("#document").after("<div class='validation' style='color:red;margin-bottom:15px;'>Please select image size less than 2MB </div>");
            }
            if (!focusSet) {
                $("#document").focus();
            }
    ...
    $('#life_certificate_form').on('change', '#ppo_id', function (e) {
        e.preventDefault();
        
        var ppo_id = $('#ppo_id').val();       
        $.post("getUserNameByPPOID/", {
            ppo_id: ppo_id
           
        }, function (data) {
            console.log(data);
           var res = JSON.parse(data);
            $('#ppo_name').val(res.name);
        });
