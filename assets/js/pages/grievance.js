...
$('#grievance_form #document').on('change', function () {
    ...
    var focusSet = true;
    if (this.files[0].size > 502400) {
        if ($("#imgError").next(".validation").length == 0) // only add if not added
        {
            $("#imgError").after("<div class='validation' style='color:red;margin-bottom:15px;'>Please select image size less than 500 KB </div>");
        }
        if (!focusSet) {
            $("#document").focus();
        }
    ...
    if ($.inArray(fileNameExt, validExtensions) == -1) {
        ...
        if ($("#imgError").next(".validation").length == 0) // only add if not added
        {
            $("#imgError").after("<div class='validation' style='color:red;margin-bottom:15px;'>Please upload .jpg /.jpeg/.png  file </div>");
        }
        if (!focusSet) {
            $("#document").focus();
        }
